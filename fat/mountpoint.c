 ...
 * @return int
 */
BFSFAT_EXPORT int fat_mountpoint_mount( const char* device_name, const char* mountpoint, bool read_only ) {
  // check parameter
  ...
  fat_fs_t* fs = malloc( sizeof( *fs ) );
  if ( ! fs ) {
    return ENOMEM;
  }
  // init fat fs
  result = fat_fs_init( fs, bdev, read_only );
  if ( EOK != result ) {
    free( fs );
 ...
 * @return int
 */
BFSFAT_EXPORT int fat_mountpoint_umount( const char* mountpoint ) {
  // loop through list try to find already mounted entry
  common_mountpoint_t* entry = common_mountpoint_by_mountpoint( mountpoint );
  if ( ! entry ) {
    return ENODEV;
