...
TEST( fat32, file_open_non_existant_file ) {
  helper_mount_fat_test_image( true, "fat32.img", "fat32", "/fat32/", FAT_FAT32 );
  // file variable
  fat_file_t file;
...
TEST( fat32, file_write_append ) {
  ...
  int result = fat_file_open2(
    &file,
    "/fat32/hello/file/write/append.txt",
    O_RDWR | O_APPEND
  );
  EXPECT_EQ( result, EOK );
  EXPECT_NE( file.cluster, 0 );
...
TEST( fat32, file_ftruncate_ro ) {
  ...
  fat_file_t file;
  memset( &file, 0, sizeof( file ) );
  int result = fat_file_open2( &file, "/fat32/hello/trunc/extend.txt", O_RDONLY );
  EXPECT_EQ( result, EOK );
  // save old size
  ...
  EXPECT_EQ( result, EOK );
  // open file again
  result = fat_file_open2( &file, "/fat32/hello/trunc/extend.txt", O_RDONLY );
  EXPECT_EQ( result, EOK );
  EXPECT_EQ( file.fsize, old_size );
  // close file again
  result = fat_file_close( &file );
...
TEST( fat32, file_ftruncate_extend_cluster ) {
  helper_mount_fat_test_image( false, "fat32.img", "fat32", "/fat32/", FAT_FAT32 );
  // open file
  fat_file_t file;
  memset( &file, 0, sizeof( file ) );
  int result = fat_file_open2( &file, "/fat32/hello/trunc/extend.txt", O_RDWR );
  EXPECT_EQ( result, EOK );
  // save old size
...
TEST( fat32, file_ftruncate_extend_cluster_times_n ) {
  helper_mount_fat_test_image( false, "fat32.img", "fat32", "/fat32/", FAT_FAT32 );
...
TEST( fat32, file_ftruncate_shrink_cluster ) {
  helper_mount_fat_test_image( false, "fat32.img", "fat32", "/fat32/", FAT_FAT32 );
  // open file
  fat_file_t file;
  memset( &file, 0, sizeof( file ) );
  int result = fat_file_open2( &file, "/fat32/hello/trunc/shrink.txt", O_RDWR );
  EXPECT_EQ( result, EOK );
  // save old size
  fat_fs_t* fs = ( fat_fs_t* )file.mp->fs;
  uint64_t cluster_size = fs->superblock.bytes_per_sector
  ...
  EXPECT_EQ( result, EOK );
  // open file again
  result = fat_file_open2( &file, "/fat32/hello/trunc/shrink.txt", O_RDWR );
  EXPECT_EQ( result, EOK );
  EXPECT_EQ( file.fsize, truncate_size );
  // call for truncate
...
TEST( fat32, file_ftruncate_extend_size_only ) {
  helper_mount_fat_test_image( false, "fat32.img", "fat32", "/fat32/", FAT_FAT32 );
  // open file
  fat_file_t file;
  memset( &file, 0, sizeof( file ) );
  int result = fat_file_open2( &file, "/fat32/hello/trunc/extend2.txt", O_RDWR );
  EXPECT_EQ( result, EOK );
  // save old size
  uint64_t old_size = file.fsize;
  // call for truncate
  ...
  EXPECT_EQ( result, EOK );
  // open file again
  result = fat_file_open2( &file, "/fat32/hello/trunc/extend2.txt", O_RDONLY );
  EXPECT_EQ( result, EOK );
  EXPECT_EQ( file.fsize, old_size * 2 );
  // close file again
  result = fat_file_close( &file );
...
TEST( fat32, file_ftruncate_shrink_size_only ) {
  helper_mount_fat_test_image( false, "fat32.img", "fat32", "/fat32/", FAT_FAT32 );
  // open file
  fat_file_t file;
  memset( &file, 0, sizeof( file ) );
  int result = fat_file_open2( &file, "/fat32/hello/trunc/shrink2.txt", O_RDWR );
  EXPECT_EQ( result, EOK );
  // save old size
  uint64_t old_size = file.fsize;
  // call for truncate
  ...
  EXPECT_EQ( result, EOK );
  // open file again
  result = fat_file_open2( &file, "/fat32/hello/trunc/shrink2.txt", O_RDONLY );
  EXPECT_EQ( result, EOK );
  EXPECT_EQ( file.fsize, old_size / 2 );
  // close file again
  result = fat_file_close( &file );
...
TEST( fat32, file_ftruncate_free_cluster_size ) {
  helper_mount_fat_test_image( false, "fat32.img", "fat32", "/fat32/", FAT_FAT32 );
  // open file
  fat_file_t file;
  memset( &file, 0, sizeof( file ) );
...
TEST( fat32, file_remove_rootdir_ro_fail ) {
  helper_mount_fat_test_image( true, "fat32.img", "fat32", "/fat32/", FAT_FAT32 );
  // try to remove directory
  int result = fat_file_remove( "/fat32/fremove.txt" );
  EXPECT_EQ( result, EROFS );
  // directory variable
  fat_file_t file;
  memset( &file, 0, sizeof( file ) );
  // open base directory
  result = fat_file_open2( &file, "/fat32/fremove.txt", O_RDONLY );
  EXPECT_EQ( result, EOK );
...
TEST( fat32, file_remove_rootdir_rw_success ) {
  helper_mount_fat_test_image( false, "fat32.img", "fat32", "/fat32/", FAT_FAT32 );
  // try to remove directory
  int result = fat_file_remove( "/fat32/fremove.txt" );
  EXPECT_EQ( result, EOK );
...
TEST( fat32, file_remove_rootdir_rw_longname ) {
  helper_mount_fat_test_image( false, "fat32.img", "fat32", "/fat32/", FAT_FAT32 );
  // try to remove directory
  int result = fat_file_remove( "/fat32/fremovelongname.txt" );
  EXPECT_EQ( result, EOK );
  // directory variable
  fat_file_t file;
  memset( &file, 0, sizeof( file ) );
  // open base directory
  result = fat_file_open2( &file, "/fat32/fremovelongname.txt", O_RDONLY );
  EXPECT_EQ( result, ENOENT );
  // unmount test image
  helper_unmount_fat_test_image( "fat32", "/fat32/" );
}

TEST( fat32, file_remove_dir_ro_fail ) {
  helper_mount_fat_test_image( true, "fat32.img", "fat32", "/fat32/", FAT_FAT32 );
  // try to remove directory
  int result = fat_file_remove( "/fat32/hello/file/remove.txt" );
  EXPECT_EQ( result, EROFS );
  // directory variable
  fat_file_t file;
  memset( &file, 0, sizeof( file ) );
  // open base directory
  result = fat_file_open2( &file, "/fat32/hello/file/remove.txt", O_RDONLY );
  EXPECT_EQ( result, EOK );
  EXPECT_EQ( file.fsize, strlen("hello world\n") );
  // close directory again
  result = fat_file_close( &file );
...
TEST( fat32, file_remove_dir_rw_success ) {
  helper_mount_fat_test_image( false, "fat32.img", "fat32", "/fat32/", FAT_FAT32 );
  // try to remove directory
  int result = fat_file_remove( "/fat32/hello/file/remove.txt" );
  EXPECT_EQ( result, EOK );
...
TEST( fat32, file_remove_dir_rw_longname ) {
  helper_mount_fat_test_image( false, "fat32.img", "fat32", "/fat32/", FAT_FAT32 );
  // try to remove directory
  int result = fat_file_remove( "/fat32/hello/file/removelongname.txt" );
  EXPECT_EQ( result, EOK );
  // directory variable
  fat_file_t file;
  memset( &file, 0, sizeof( file ) );
  // open base directory
  result = fat_file_open2( &file, "/fat32/hello/file/removelongname.txt", O_RDONLY );
  EXPECT_EQ( result, ENOENT );
  // unmount test image
  helper_unmount_fat_test_image( "fat32", "/fat32/" );
}

TEST( fat32, file_move_rootdir_ro_fail ) {
  helper_mount_fat_test_image( true, "fat32.img", "fat32", "/fat32/", FAT_FAT32 );
  // try to remove directory
  int result = fat_file_move( "/fat32/fmove.txt", "/fat32/fmove2.txt" );
  EXPECT_EQ( result, EROFS );
  // directory variable
  fat_file_t file;
  memset( &file, 0, sizeof( file ) );
  // open base directory
  result = fat_file_open2( &file, "/fat32/fmove.txt", O_RDONLY );
  EXPECT_EQ( result, EOK );
  EXPECT_EQ( file.fsize, strlen("hello world\n") );
  // open base directory
  result = fat_file_open2( &file, "/fat32/fmove2.txt", O_RDONLY );
  EXPECT_EQ( result, ENOENT );
  // close directory again
  result = fat_file_close( &file );
  EXPECT_EQ( result, EOK );
...
TEST( fat32, file_move_rootdir_target_exist_fail ) {
  helper_mount_fat_test_image( false, "fat32.img", "fat32", "/fat32/", FAT_FAT32 );
  // try to remove directory
  int result = fat_file_move( "/fat32/fmove.txt", "/fat32/world.txt" );
  ...
  EXPECT_EQ( result, EOK );
  // open base directory
  result = fat_file_open2( &file, "/fat32/world.txt", O_RDONLY );
  EXPECT_EQ( result, EOK );
  // close directory again
  result = fat_file_close( &file );
  EXPECT_EQ( result, EOK );
...
TEST( fat32, file_move_rootdir_source_not_exist_fail ) {
  helper_mount_fat_test_image( false, "fat32.img", "fat32", "/fat32/", FAT_FAT32 );
  // try to remove directory
  int result = fat_file_move( "/fat32/fmoveinvalid.txt", "/fat32/world2.txt" );
  EXPECT_EQ( result, ENOENT );
  // directory variable
  fat_file_t file;
  memset( &file, 0, sizeof( file ) );
...
TEST( fat32, file_move_rootdir_rw_short_name_success ) {
  helper_mount_fat_test_image( false, "fat32.img", "fat32", "/fat32/", FAT_FAT32 );
  // try to remove directory
  int result = fat_file_move( "/fat32/fmove.txt", "/fat32/fmove2.txt" );
  EXPECT_EQ( result, EOK );
  ...
  result = fat_file_move( "/fat32/fmove2.txt", "/fat32/fmove.txt" );
  EXPECT_EQ( result, EOK );
  // open base directory
  result = fat_directory_open( &dir, "/fat32/" );
  EXPECT_EQ( result, EOK );
...
TEST( fat32, file_move_rootdir_rw_long_name_success ) {
  helper_mount_fat_test_image( false, "fat32.img", "fat32", "/fat32/", FAT_FAT32 );
  // try to remove directory
  int result = fat_file_move( "/fat32/fmovelongname.txt", "/fat32/fmovelongname2.txt" );
  EXPECT_EQ( result, EOK );
  // directory variable
  fat_directory_t dir;
  memset( &dir, 0, sizeof( dir ) );
  ...
  EXPECT_EQ( result, EOK );
  // try to revert directory move
  result = fat_file_move( "/fat32/fmovelongname2.txt", "/fat32/fmovelongname.txt" );
  EXPECT_EQ( result, EOK );
  // open base directory
  result = fat_directory_open( &dir, "/fat32/" );
  EXPECT_EQ( result, EOK );
...
TEST( fat32, file_move_dir_ro_fail ) {
  helper_mount_fat_test_image( true, "fat32.img", "fat32", "/fat32/", FAT_FAT32 );
  // try to remove directory
  int result = fat_file_move( "/fat32/hello/file/fmove.txt", "/fat32/hello/file/fmove2.txt" );
  EXPECT_EQ( result, EROFS );
  // directory variable
  fat_file_t file;
  memset( &file, 0, sizeof( file ) );
  // open base directory
  result = fat_file_open2( &file, "/fat32/hello/file/fmove.txt", O_RDONLY );
  EXPECT_EQ( result, EOK );
  EXPECT_EQ( file.fsize, strlen("hello world\n") );
  // close again
  result = fat_file_close( &file );
  EXPECT_EQ( result, EOK );
  // open base directory
  result = fat_file_open2( &file, "/fat32/hello/file/fmove2.txt", O_RDONLY );
  EXPECT_EQ( result, ENOENT );
  // unmount test image
  helper_unmount_fat_test_image( "fat32", "/fat32/" );
}

TEST( fat32, file_move_dir_target_exist_fail ) {
  helper_mount_fat_test_image( false, "fat32.img", "fat32", "/fat32/", FAT_FAT32 );
  // try to remove directory
  int result = fat_file_move( "/fat32/hello/file/fmove.txt", "/fat32/hello/world.txt" );
  EXPECT_EQ( result, EEXIST );
  // directory variable
  fat_file_t file;
  memset( &file, 0, sizeof( file ) );
  ...
  EXPECT_EQ( result, EOK );
  // open base directory
  result = fat_file_open2( &file, "/fat32/hello/world.txt", O_RDONLY );
  EXPECT_EQ( result, EOK );
  // close again
  result = fat_file_close( &file );
  EXPECT_EQ( result, EOK );
...
TEST( fat32, file_move_dir_source_not_exist_fail ) {
  helper_mount_fat_test_image( false, "fat32.img", "fat32", "/fat32/", FAT_FAT32 );
  // try to remove directory
  int result = fat_file_move( "/fat32/hello/file/fmoveinvalid.txt", "/fat32/world2.txt" );
  EXPECT_EQ( result, ENOENT );
  // directory variable
  fat_file_t file;
  memset( &file, 0, sizeof( file ) );
...
TEST( fat32, file_move_dir_rw_short_name_success ) {
  helper_mount_fat_test_image( false, "fat32.img", "fat32", "/fat32/", FAT_FAT32 );
  // try to remove directory
  int result = fat_file_move( "/fat32/hello/file/fmove.txt", "/fat32/hello/file/fmove2.txt" );
  EXPECT_EQ( result, EOK );
  ...
  memset( &dir, 0, sizeof( dir ) );
  // open base directory
  result = fat_directory_open( &dir, "/fat32/hello/file/" );
  EXPECT_EQ( result, EOK );
  // get by name
  result = fat_directory_entry_by_name( &dir, "fmove.txt" );
  EXPECT_EQ( result, ENOENT );
  ...
  EXPECT_EQ( result, EOK );
  // try to revert directory move
  result = fat_file_move( "/fat32/hello/file/fmove2.txt", "/fat32/hello/file/fmove.txt" );
  EXPECT_EQ( result, EOK );
  // open base directory
  result = fat_directory_open( &dir, "/fat32/hello/file/" );
  EXPECT_EQ( result, EOK );
...
TEST( fat32, file_move_dir_rw_long_name_success ) {
  helper_mount_fat_test_image( false, "fat32.img", "fat32", "/fat32/", FAT_FAT32 );
  // try to remove directory
  int result = fat_file_move( "/fat32/hello/file/fmovelongname.txt", "/fat32/hello/file/fmovelongname2.txt" );
  EXPECT_EQ( result, EOK );
  // directory variable
  fat_directory_t dir;
  memset( &dir, 0, sizeof( dir ) );
  ...
  EXPECT_EQ( result, EOK );
  // try to revert directory move
  result = fat_file_move( "/fat32/hello/file/fmovelongname2.txt", "/fat32/hello/file/fmovelongname.txt" );
  EXPECT_EQ( result, EOK );
  // open base directory
  result = fat_directory_open( &dir, "/fat32/" );
  EXPECT_EQ( result, EOK );
